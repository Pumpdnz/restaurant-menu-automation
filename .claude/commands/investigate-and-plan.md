---
description: Investigate the codebase to gather an understanding of the current project in relation to the changes being planned
argument-hint: [path-to-project-docs]
---

# Investigate and Plan

Follow the `Workflow` to research the relevant files within the codebase and create the documentation within the `PATH_TO_PROJECT_DOCS` folder, then `Report` your understanding to the user
...
- Use ultrathink to consider the content of the `USER_PROMPT` and then begin researching the codebase, database and if required, use webfetch to read external documentation. Gather a complete understanding of the current problem or feature being implemented.
- Create all relevant documentation files within the `PATH_TO_PROJECT_DOCS` folder, following the `files_to_create` guide below. If no folder currently exists at this path, create it first. Use your own initiative to create the relevant files as per the project described in the `USER_PROMPT`.
