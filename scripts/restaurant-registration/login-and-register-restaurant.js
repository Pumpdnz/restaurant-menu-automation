...
/**
 * Login and Restaurant Registration Script for CloudWaitress Admin Portals
 *
 ...
 * and country-specific settings (locale, timezone, currency, etc.)
 *
 * Usage:
 ...
 *   --country=<code>         Country code: NZ, AU, US, GB, CA (default: NZ)
 *
 ...
 *
 * Example (AU - custom admin URL):
 *   node login-and-register-restaurant.js --email=test@example.com --password=Test789! --name="Pizza Palace" \
 *     --admin-url=https://admin.ozorders.com.au --country=AU
 *
...
const countryCode = (getArg('country') || DEFAULT_COUNTRY).toUpperCase();

// Get country-specific configuration
const countryConfig = getCountryConfig(countryCode);

// Get timezone - can be overridden via argument, falls back to country default
...
const takeScreenshot = async (page, name) => {
  return sharedTakeScreenshot(page, `login-restaurant-${name}`, SCREENSHOT_DIR);
};
...
async function loginAndRegisterRestaurant() {
  ...
  console.log(`  Country: ${countryCode} (${countryConfig.name})`);
  ...
  console.log('Country Settings:');
  console.log(`  Locale: ${countryConfig.locale}`);
  console.log(`  Timezone: ${timezone}${timezone !== countryConfig.timezoneDisplay ? ' (custom)' : ''}`);
  ...
  try {
    ...
    try {
      ...
      await page.waitForTimeout(500);
      await page.keyboard.type(countryConfig.locale);
      console.log(`  ✓ Typed "${countryConfig.locale}"`);
      ...
      console.log(`  ✓ Selected ${countryConfig.locale} locale`);
    } catch (error) {
    ...
    // STEP 13: Set Timezone (country-specific)
    ...
    // STEP 14: Set Currency (country-specific)
    ...
    try {
      ...
      console.log(`  ✓ Typed "${countryConfig.currency}"`);
      ...
      console.log(`  ✓ Selected ${countryConfig.currency} currency`);
    } catch (error) {
    ...
    try {
      ...
      await page.waitForURL(`**/${ADMIN_HOSTNAME}/**`, { timeout: 30000 });
    ...
    if (navigationSuccess) {
      ...
      if (restaurantIndex >= 0) {
        ...
        if (restaurantIdMatch) {
          ...
          // Extract the subdomain base from admin URL (e.g., admin.pumpd.co.nz -> pumpd.co.nz)
          const subdomainBase = ADMIN_HOSTNAME.replace(/^admin\./, '');
          ...
          console.log(`  ✓ System locale (${countryConfig.locale})`);
          ...
          console.log(`  Subdomain: ${subdomain}.${subdomainBase}`);
          ...
          // Output for parsing
          console.log('\nSubdomain:', `${subdomain}.${subdomainBase}`);
