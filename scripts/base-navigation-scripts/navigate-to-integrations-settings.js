 ...
 * 
 * This script logs into the admin portal and navigates to Integrations settings tab
 * 
 * Usage:
 *   node navigate-to-integrations-settings.js --email=<email>
 * 
 * Options:
...
if (!email) {
  ...
  console.error('Usage: node navigate-to-integrations-settings.js --email="email@example.com"');
  process.exit(1);
}

// Utility function for screenshots
const takeScreenshot = async (page, name) => {
  const screenshotPath = path.join(__dirname, 'screenshots', `integrations-settings-${name}-${Date.now()}.png`);
  await fs.mkdir(path.dirname(screenshotPath), { recursive: true });
  await page.screenshot({ path: screenshotPath, fullPage: true });
  console.log(`📸 Screenshot: ${screenshotPath}`);
};

async function navigateToIntegrationsSettings() {
  console.log('🚀 Starting Navigation to Integrations Settings...\n');
  ...
  try {
    ...
    // STEP 4: Click on Integrations tab
    ...
    try {
      ...
      // Verify we're on the Integrations tab by checking for Integrations-specific content
      const integrationsContent = await page.locator('text=/Google|Xero|Delivery Platforms/i').count();
      if (integrationsContent > 0) {
        console.log('  ✓ Integrations tab content detected');
      } else {
        console.log('  ⚠️ Could not verify Integrations content, checking URL...');
      }
      
      console.log('  ✓ Successfully navigated to Integrations tab');
      
    } catch (error) {
      console.error('  ❌ Failed to navigate to Integrations tab:', error.message);
      await takeScreenshot(page, 'error-integrations-tab');
      throw error;
    }
    
    await takeScreenshot(page, '05-integrations-settings');
    
    console.log('\n✅ Successfully navigated to Integrations Settings!');
...
// Run the script
navigateToIntegrationsSettings().catch(error => {
