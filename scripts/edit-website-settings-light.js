...
if (!email || !password || !primaryColor || !hasHeadContent || !hasBodyContent || !restaurantName) {
  ...
  console.error('\nExample (file paths):');
  console.error('node edit-website-settings-light.js --email="test@example.com" --password="Password123!" --name="Test Restaurant" --primary="#A47F20" --head="../generated-code/restaurant/head-injection.html" --body="../generated-code/restaurant/body-injection.html"');
  console.error('\nExample (environment variables):');
  console.error('HEAD_INJECTION_CONTENT=<base64> BODY_INJECTION_CONTENT=<base64> node edit-website-settings-light.js --email="..." --password="..." --name="..." --primary="..." --head-from-env --body-from-env');
  process.exit(1);
}
  ...
  try {
    ...
    const colorFields = [
      {
        ...
        value: boxTextColorArg || secondaryColor || primaryColor  // Use configured color, or secondary if available, else primary
      },
      {
    ...
    if (logoPath) {
      ...
      try {
        ...
        await page.waitForTimeout(500);
        const navTextColor = navTextColorArg || secondaryColor || primaryColor;  // Use configured color, or secondary if available, else primary
      ...
      try {
        ...
        await page.click(navTextPickerSelector);
        await page.waitForTimeout(500);
        const navTextColor = navTextColorArg || secondaryColor || primaryColor;
        await page.fill(navTextInputSelector, navTextColor);
        console.log(`  ✓ Set Nav Bar Text to ${navTextColor}`);
        await page.keyboard.press('Escape');
        await page.waitForTimeout(500);
    ...
    if (faviconToUpload) {
      ...
      console.log('\n📋 STEP 14: Skipping favicon upload (no logo provided)');
    }
