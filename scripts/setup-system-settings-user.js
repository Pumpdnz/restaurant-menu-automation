...
/**
 * Setup System Settings (User Credentials Version)
 *
 * This script logs into the admin portal using user credentials and configures System settings
 *
 * Usage:
 *   node setup-system-settings-user.js --email=<email> --password=<password> --name=<restaurant_name> [options]
 *
 * Options:
...
const receiptLogoPath = getArg('receipt-logo') || getArg('receiptLogo');
...
if (!email || !password || !restaurantName) {
  ...
  console.error('Usage: node setup-system-settings-user.js --email="email@example.com" --password="password" --name="Restaurant Name"');
  process.exit(1);
}
...
async function setupSystemSettings() {
  ...
  let webhookSecret = '';
  let receiptPrinterApiKey = '';
  let kitchenPrinterApiKey = '';
  ...
  try {
    ...
    if (restaurantIndex >= 0) {
      console.log(`  ✅ Best match: "${bestMatch.name}" (${bestMatch.reason})`);

      // Try multiple selectors to click the Manage button
      let clicked = false;
      ...
      if (await manageButton.count() > 0) {
        ...
        clicked = true;
        console.log(`  ✓ Clicked Manage button using standard selector`);
      }

      // Try view-store pattern if standard didn't work
      if (!clicked) {
      ...
      if (!clicked) {
        ...
        if (await allManageButtons.count() > restaurantIndex) {
          ...
          clicked = true;
          console.log(`  ✓ Clicked Manage button using index fallback`);
        }
      }
      ...
      throw new Error(`Restaurant "${restaurantName}" not found in list`);
    }
    ...
    if (currentPhone) {
      ...
      console.log(`  ⚠️ No phone number found in field`);
    }
    ...
    try {
      ...
      receiptPrinterApiKey = await apiKeyElement.textContent();
      console.log(`  📋 API Key: ${receiptPrinterApiKey}`);
    } catch (error) {
    ...
    try {
      ...
      kitchenPrinterApiKey = await apiKeyElement2.textContent();
      console.log(`  📋 API Key: ${kitchenPrinterApiKey}`);
    } catch (error) {
      console.log('  ⚠️ Could not extract API key:', error.message);
    }
    ...
    // Output parseable result data for backend
    console.log('\n### SCRIPT_RESULT_START ###');
    console.log(JSON.stringify({
      success: true,
      webhookSecret: webhookSecret || null,
      receiptPrinterKey: receiptPrinterApiKey || null,
      kitchenPrinterKey: kitchenPrinterApiKey || null,
